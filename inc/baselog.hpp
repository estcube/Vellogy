...
class log_slice;
template<template<class> class T, class E>
requires loggable<T, E> class log;
...
class base_log {
  ...
  uint32_t entries_added;   ///< How many log entries have been added to the log file during current logging
                            ///< session
   ...
   */
  void write_to_queue(auto var_address, ///< [in] From where will data be written to the queue
   ...
   */
  void deserialize_meta_info(uint8_t *metafile ///< [in] Pointer to the metafile that holds the info to be deserialized
  ) {
   ...
   * Dummy
   */
  void write_to_index(time_t timestamp, ///< [in] Timestamp in the beginning of a log entry
   ...
   */
  base_log(uint8_t *file ///< [in] Pointer to the file where datapoints will be saved
  ) : file(file), file_size{0}, entries_added{0}, metafile(NULL), indexfile(NULL), indexfile_size{0}, queue_len{0}
  {}

  /**
   * Initialize the log (from a metafile) held in the given file
   */
  base_log(uint8_t *metafile, ///< [in] Pointer to the file where metainfo will be saved
  ...
  ) : file(file), entries_added{0}, metafile(metafile), indexfile(indexfile), queue_len{0} {
