...
namespace eclog {
typedef uint32_t (*entry_search_fun)(
  uint8_t *,
  uint32_t,
  uint8_t,
  time_t,
 ...
 */
static entry_location_t find_entry_location(uint8_t *file, ///< [in] Pointer to the log file where we search the entry
  ...
  entry_search_fun find_log_entry,                         ///< [in] Function used to search for the log entry when the
                                                           ///< appropriate index entry has been found
  uint32_t first_index,                                    ///< [in] Number of the first index entry of the interval the
                                                           ///< log entry is searched from
  uint32_t second_index,                                   ///< [in] Number of the second index entry of the interval
                                                           ///< the log entry is searched from
  time_t timestamp,                                        ///< [in] Timestamp to be searched
  bool succeeding                                          ///< [in] Do we want the entry containing the timestamp or
                                                           ///< the entry after it
) {
  uint32_t location;
  ...
  if (first_index == second_index) {
    // If first_index == second_index, then we have no information about the interval in which the timestamp lies, so we
  ...
  while (true) {
    ...
    bool ts_exact_match = timestamp == middle_ts; // Timestamp happened to be an index entry timestamp
                                                  // (quite unlikely)

    if ((second_index - first_index == 1) || ts_exact_match) { // The binary search is complete
      uint32_t search_index = ts_exact_match ? (succeeding ? middle_index + 1 : middle_index) : (timestamp < second_ts ?
        second_index : UINT_MAX);
      ...
      if (search_index != UINT_MAX)
        std::memcpy(
          &search_location,
          indexfile + search_index * LOG_INDEX_ENTRY_SIZE + sizeof(time_t),
          sizeof(uint32_t)
        );

      // If the timestamp happened to be an exact index entry timestamp, the search_location already is the location of
      // the timestamp in the log file
      location = ts_exact_match && !succeeding ? search_location : find_log_entry(
        file,
        file_size,
        datapoint_size,
        timestamp,
...
log_slice<T, E> make_log_slice(uint8_t *file, ///< [in] Pointer to the log file where we search the entry
  uint32_t file_size,                         ///< [in] Size of the log file
  uint8_t *indexfile,                         ///< [in] Pointer to the log index file
  uint32_t indexfile_size,                    ///< [in] Size of the log index file
  ...
  entry_location_t start = find_entry_location(
    file,
    file_size,
    file_break,
    indexfile,
  ...
  entry_location_t end = find_entry_location(
    file,
    file_size,
    file_break,
    indexfile,
...
log_slice<T, E> make_log_slice(uint8_t *file, ///< [in] Pointer to the log file where we search the entry
  ...
  log_slice<T, E> slice = make_log_slice<T, E>(
    file,
    file_size,
    indexfile,
    indexfile_size,
