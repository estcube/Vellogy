...
class circular_log : public simple_log<T> {
    private:
        ...
        void write_to_file(
            ...
            uint32_t first_bytes = size <= this->file_capacity - this->file_break ? size : this->file_capacity - this->file_size;
    ...
    public:
        ...
        void log(
            ...
            this->switch_buffers();
            // Note that this signifies the total number of entries added to the file, not the number of entries currently in the file
            this->entries_added++;
        ...
        static uint32_t find_log_entry(
            ...
            while (!done) {
                ...
                reading_location = (int64_t)reading_location - (sizeof(time_t) + datapoint_size) >= LOG_MANDATORY_DECODE_INFO_SIZE
