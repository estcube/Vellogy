 ...
 * * LOG_FORMATSTRING_LEN               - How many bytes are allocated for holding a timestamp or a datapoint
 * formatstring (for decoding)
 ...
 *
 * @tparam T a subclass of BaseLog on which all the functions are actually called (determines the inner log file
 * structure)
 * @tparam E the type of data held in the log
 */
template<template<class> class T, class E>
requires loggable<T, E>
class log {
  private:
   ...
   */
  log(T<E> *log_obj ///< [in] A BaseLog subclass object that the Log methods will be invoked on
  ) {
    this->obj = log_obj;
  }
   ...
   */
  void write(E &data, ///< [in] Datapoint to be logged
   ...
   */
  log_slice<T, E> slice(time_t start_ts, ///< [in] Starting point of the chosen time period
